...
public class Solver : MonoBehaviour
{
    ...
    //---------------------------------------
    public GameObject collisionSphere0;//不要超过十个
    public GameObject collisionSphere1;




    public Vector3 boxCenter = new Vector3(0f,0f,0f);
    ...
    private void OnDrawGizmos()
    {
        ...
        Gizmos.DrawWireCube(boxCenter + new Vector3(0f,50f,0f), new Vector3(100f*boxScale.x,100f*boxScale.y,100f * boxScale.z));
    }
    ...
    void UpdateParams() {
        ...
        //box planes 和 wave planes 共同构成 域


        boxPlanes[0] = GetPlaneEq(new Vector3(0, 0+boxCenter.y, 0), Vector3.up);
        boxPlanes[1] = GetPlaneEq(new Vector3(0, 100*boxScale.y+boxCenter.y, 0), Vector3.down);
        boxPlanes[2] = GetPlaneEq(new Vector3(-50*boxScale.x+ boxCenter.x, 0, 0), Vector3.right);
        boxPlanes[3] = GetPlaneEq(new Vector3(50 * boxScale.x + boxCenter.x, 0, 0), Vector3.left);
        boxPlanes[4] = GetPlaneEq(new Vector3(0, 0, -50 * boxScale.z + boxCenter.z), Vector3.forward);
        boxPlanes[5] = GetPlaneEq(new Vector3(0, 0, 50 * boxScale.z+ boxCenter.z), Vector3.back);
        ...
        */
        ...
        /*
        //将碰撞球体gameobject组的信息提取出来,制作出 raidus 和 position 两个 list,然后转换为 array
        List<Vector4> spherePosistion_ = new List<Vector4>();
        for (int i = 0; i < collisionSphere.Length; i++)
        {
            spherePosistion_.Add(collisionSphere[i].transform.position);
        }
        Vector4[] spherePosition =  spherePosistion_.ToArray();

        List<float> sphereRadius_ = new List<float>();
        for (int i = 0; i < collisionSphere.Length; i++)
        {
            sphereRadius_.Add(collisionSphere[i].transform.localScale.x / 2);
        }
        float[] sphereRadius = sphereRadius_.ToArray();

        
        solverShader.SetInt("sphereNum", collisionSphere.Length);//将碰撞球体的个数传递过去
        solverShader.SetFloats("sphereRadius", sphereRadius);
        solverShader.SetVectorArray("spherePosition", spherePosition);
        */
    ...
    void Start() {
        ...
        Vector3 sourceCenter =  GetComponent<SourceGizemos>().sourceCenter;
        /*
        ...
        //Vector3 pos;
        //for (int i = 0; i < (int)(initScale.z / initSize); i++)
        //{
        //    for (int j = 0; i < (int)(initScale.y / initSize); j++)
        //    {
        //        for (int k = 0; k < (int)(initScale.x / initSize); k++)
        //        {
        //            pos = new Vector3(k, j, i) - initScale*0.5f + sourceCenter;
        //            particles[i].pos = pos;
        ...
        for (int i = 0; i < numParticles; i++)
        {
            Vector3 pos = new Vector3(
                Random.Range(0f, 1f) * initScale.x - initScale.x * 0.5f,
                Random.Range(0f, 1f) * initScale.y - initScale.y * 0.5f,
                Random.Range(0f, 1f) * initScale.z - initScale.z * 0.5f
            );
    ...
    void Update() {
        ...
        //solverShader.SetInt("sphereNum", collisionSphere0);//将碰撞球体的个数传递过去

        solverShader.SetFloats("sphereRadius0", collisionSphere0.transform.localScale.x/2);
        solverShader.SetVector("spherePosition0", collisionSphere0.transform.position);

        solverShader.SetFloats("sphereRadius1", collisionSphere1.transform.localScale.x / 2);
        solverShader.SetVector("spherePosition1", collisionSphere1.transform.position);
